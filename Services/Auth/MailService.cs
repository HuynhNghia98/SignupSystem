...
namespace SignupSystem.Services.Auth
{
	public class MailService : SmtpClient, Interfaces.IMailService
	{
		...
		public async Task SendEmailAsync(string lastName, string firstName, string to, string subject, string body)
		{
			var emailMessage = new MimeMessage();
			emailMessage.From.Add(new MailboxAddress(_mailSettings.DisplayName, _mailSettings.Mail));
			emailMessage.To.Add(new MailboxAddress(lastName + " " + firstName, to));
